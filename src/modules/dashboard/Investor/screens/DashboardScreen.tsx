...
const dashboardData = {
  ...
  recent_activities: [
    { id: 1, icon: 'arrow-down-right', text: 'Payout received', date: 'Jul 1', amount: '+$1,200' },
    { id: 2, icon: 'arrow-up-right', text: 'Invested in "Tech Fund"', date: 'Jun 28', amount: '-$500' },
  ],
  upcoming_payouts: [
    { id: 1, icon: 'calendar', text: 'Payout scheduled', date: 'Jul 15', amount: '$2,000' },
  ],
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statCardGrid}>
          {statCards.map((card, idx) => (
            <View key={card.label} style={[styles.statCardLarge, { backgroundColor: card.bg, marginRight: idx % 2 === 0 ? 8 : 0, marginLeft: idx % 2 === 1 ? 8 : 0 }]}> 
              <View style={{ flex: 1 }}>
                <Text style={styles.statLabelLarge}>{card.label}</Text>
                <Text style={styles.statValueLarge}>{card.value}</Text>
              </View>
              <Feather name={card.icon as any} size={38} color="#888" style={{ alignSelf: 'flex-end' }} />
            </View>
          ))}
        </View>
