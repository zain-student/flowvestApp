  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchInvestments.fulfilled, (state, action) => {
        ...
        const active = action.payload.filter((inv:Investment)=>inv.status === "active").length;
        const avgROI =
      action.payload.reduce(
        (sum:number, inv:{expected_return_rate:any}) => sum + parseFloat(inv.expected_return_rate || 0),
        0
      ) / total || 0;
